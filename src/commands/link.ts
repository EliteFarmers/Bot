...
const command = new EliteCommand(linkCommand);
...
async function execute(interaction: ChatInputCommandInteraction) {
	...
	try {
		...
		if (!response.ok) {
			const embed = ErrorEmbed('Failed to Link Account!')
				...
				.addFields({
					...
					value: 'Please go to [eliteskyblock.com/profile](https://elitesb.gg/profile) and remove your account there.',
				});
			return interaction.editReply({ embeds: [embed] });
		}
		...
		const embed = EliteEmbed()
			...
			.setDescription(
				`Your account has been linked to ${escapeIgn(playerName)}! You can now use the ${commandReferences.get('weight')} command without entering your username!`,
			)
			.addFields({
				name: 'Want to unlink your account?',
				value:
					'Please go to [eliteskyblock.com/profile](https://elitesb.gg/profile) and remove your account there. This is also where you can change which account is primary.',
			});
