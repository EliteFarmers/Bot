...
async function execute(interaction: ChatInputCommandInteraction) {
	const embed = EliteEmbed()
		...
		.addFields([
			{ 
				...
				value: 'Badges are cosmetic profile decorations players can display on the website. You ~~must~~ should try to collect them all!'
			}, {
				name: 'Elite Farmer Badge',
				value: 'Awarded to players who have obtained the Elite Farmer role in the [Elite Farmers](https://elitebot.dev/discord) Discord server.',
				inline: true
			}, {
				name: 'Donator Badge',
				value: 'Awarded to players who have donated to the project. [Donate Here!](https://elitebot.dev/donate) (Message `kaeso.dev` with proof for badge)',
				inline: true
			}, {
				name: 'Patron of the Arts Badge',
				value: 'Awarded to players who have bought anything from [Lumini\'s Esty Shop](https://elitebot.dev/stickers) (Message `kaeso.dev` with proof for badge)',
				inline: true
			}, { 
	...
	const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
		new ButtonBuilder()
			.setCustomId('claimelite')
			.setLabel('Claim Elite Farmer Badge')
	...
	const collector = reply.createMessageComponentCollector({ filter, time: 60_000, componentType: ComponentType.Button });
	...
	collector.on('collect', async (i: Interaction) => {
		...
		if (i.customId === 'claimelite') {
			...
			if (!account?.id) {
				const errorEmbed = ErrorEmbed('Failed to fetch account!')
					.setDescription('Please make sure your account is linked with `/verify` and try again.');
			...
			const newEmbed = EliteEmbed()
				...
				.setDescription(`Your primary Minecraft account is ${account.name} (${account.id}), if this is incorrect please change it in your [profile settings](https://elitebot.dev/profile) and try again.`);

			const row = new ActionRowBuilder<ButtonBuilder>().addComponents(
				new ButtonBuilder()
					.setCustomId('confirmclaim')
					.setLabel('Confirm')
		...
		if (i.customId === 'confirmclaim') {
			const newEmbed = EliteEmbed()
				...
				.setDescription('Your badge claim has been submitted! Please try again if it doesn\'t appear on your website profile within a few minutes, and contact `kaeso.dev` if it doesn\'t appear within an hour.');
