/**
 * Generated by orval v8.6.2 🍺
 * Do not edit manually.
 * Elite API
 * A backend API for https://eliteskyblock.com/ that provides Hypixel Skyblock data.
<br><br>
Use of this API requires following the [Elite API TOS](https://eliteskyblock.com/apiterms). This API is not affiliated with Hypixel or Mojang.
 * OpenAPI spec version: admin-v1
 */
import type { WeightStyleBackgroundRectDto } from './WeightStyleBackgroundRectDto';
import type { WeightStylePositionDto } from './WeightStylePositionDto';

export interface WeightStyleBackgroundDto {
	size?: WeightStylePositionDto | null;
	/** @nullable */
	fill?: string | null;
	/** @nullable */
	align?: string | null;
	/** @nullable */
	rects?: WeightStyleBackgroundRectDto[] | null;
	/** @nullable */
	imageUrl?: string | null;
	/** @nullable */
	radius?: number | null;
	/** @nullable */
	opacity?: number | null;
}
